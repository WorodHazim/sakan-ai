...
const T = {
  EN: {
    ...
    modalCancel: "Cancel",
    btnApproveRec: "Confirm Approval",
...
const initialCases = Object.values(MOCK_CASES).map(c => ({
  caseData: c,
  recommendation: runDecisionAgent(c),
}));
...
export default function OfficerWorkspace() {
  ...
  const [modalType, setModalType] = useState<"approve" | "notify" | "requestDoc" | "assignHuman" | null>(null);
  ...
  const escalatedCount = cases.filter(c => c.recommendation.status === "Escalated").length;
  ...
  const filteredCases = cases.filter(pkg => {
    ...
    const matchesFilter = (() => {
      ...
      if (activeFilterId === "approved")
        return ["Approved", "Officer Approved Recommendation"].includes(pkg.recommendation.status);
      if (activeFilterId === "additional_info")
        return ["Additional Information Required", "Waiting for Applicant Documents"].includes(pkg.recommendation.status);
      if (activeFilterId === "human_review")
        return ["Human Review Required", "Assigned to Senior Officer"].includes(pkg.recommendation.status);
      ...
      if (activeFilterId === "missing_documents") 
        return pkg.recommendation.status === "Additional Information Required";
      return true;
    })();
    ...
    <div className="min-h-screen bg-sakan-bg p-4 md:p-8 lg:p-12 pb-24" dir={isAr ? "rtl" : "ltr"}>
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-sakan-navy text-white p-8 rounded-3xl shadow-lg">
        ...
        <div className="flex items-center gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold text-sakan-gold">{totalCases}</div>
            <div className="text-xs uppercase tracking-wider text-white/60 font-semibold mt-1">{t.totalCases}</div>
          </div>
          <div className="w-px h-10 bg-white/20" />
          <div className="text-center">
            <div className="text-3xl font-bold text-sakan-danger">{escalatedCount}</div>
            <div className="text-xs uppercase tracking-wider text-white/60 font-semibold mt-1">{t.escalated}</div>
          </div>
      ...
      <div className="space-y-4 mt-6">
        {filteredCases.length === 0 && (
          <div className="text-center text-gray-500 py-10">{t.emptyState}</div>
        )}
        {filteredCases.map((pkg, i) => (
          <motion.div
          ...
          >
            ...
            <div className="shrink-0 flex flex-col justify-center gap-2 border-t md:border-t-0 md:border-l border-sakan-border pt-4 md:pt-0 md:pl-6 w-full md:w-48">
              {pkg.caseData.caseId === "CASE-A" && (
                <>
              ...
              {pkg.caseData.caseId === "CASE-B" && (
                <>
                  ...
                  <button onClick={() => handleMarkWaiting(pkg.caseData.caseId)} className="w-full inline-flex items-center justify-center bg-sakan-navy text-white hover:bg-sakan-navy/90 px-4 py-1.5 rounded-xl font-bold transition-all text-xs shadow-sm">
                    {t.markWaitingBtn}
                  </button>
                </>
              )}
              {pkg.caseData.caseId === "CASE-C" && (
              ...
              <Link href={`/officer/report/${pkg.caseData.caseId}`} className="w-full inline-flex items-center justify-center gap-2 bg-white border-2 border-sakan-navy text-sakan-navy hover:bg-sakan-navy hover:text-white px-4 py-2 rounded-xl font-bold transition-all text-sm">
                {t.openReport}
