...
export function checkHasBlockingDocumentIssues(caseData: CaseData, docVal: any): boolean {
  ...
  return isExpired || isMismatch || isBankInconsistent || isStampMissing || isSignatureMissing || isMissingDoc || isOcrLowConfidence || isDocValFailed;
}
...
export function buildAgentChain(
  ...
  if (max20PercentDeduction - currentInstallment > 0) {
    proposedArrearsPayment = Math.min(max20PercentDeduction - currentInstallment, arrearsAmount);
  }
  const proposedTotalDeduction = currentInstallment + proposedArrearsPayment;
  const proposedDurationMonths = proposedArrearsPayment > 0 ? Math.ceil(arrearsAmount / proposedArrearsPayment) : null;
  ...
  const hasPolicyFailure = isCapFailed || isRepaymentPeriodFailed || isObligationsFailed || isIncomePerMemberFailed || isActiveRequestFailed;
